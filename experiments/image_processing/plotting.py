...
def create_evaluation_plot(loading_path, path_of_experiment):
    ...
    ax_1.set(title=f'Conv. Rate', xlabel='$r(\\zeta_n, p_n)$')
    ax_1.grid('on')
    ax_1.legend()

    # Plot stopping times
    alpha = 0.5
    ax_2.hist(stopping_times_pac, alpha=alpha, bins=20, color=colors['stopping_time'],
              edgecolor=colors['stopping_time'])
    ...
    ax_0.set(title=f'Loss over Iterations', xlabel='$t$', ylabel='$\\ell(x^{(t)}, p) - \\ell^*_p$')
    ax_0.legend()
    ax_0.grid('on')
    ax_0.set_yscale('log')
