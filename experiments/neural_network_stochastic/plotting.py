...
def create_evaluation_plot(loading_path, path_of_experiment):
    ...
    losses_std = np.load(loading_path + 'losses_of_adam.npy')
    losses_pac = np.load(loading_path + 'losses_of_learned_algorithm.npy')
    stopping_times_pac = np.load(loading_path + 'stopping_times_of_learned_algorithm.npy')
    ...
    # Post-processing: If we had used a stopping criterion for Adam as well, it would have stopped at a loss <= 0.75
    ...
    # Plot stopping times
    alpha = 0.5
    ax_2.hist(stopping_times_pac, alpha=alpha, bins=20,
              color=colors['stopping_time'], edgecolor=colors['stopping_time'])
    ...
    ax_0.axvline(n_train, 0, 1, color=colors['pac'], linestyle='dashdot', alpha=0.5, label='$t_{train}$')

    # Plot optimal loss, which is 1.0 here, because we added standard Gaussian noise to the regression function
    ax_0.axhline(1.0, 0, 1, color=colors['other'], linestyle='dashed', label='$c(g(x), y_{\\rm{obs}})$')

    # Finalize plot for loss over iterations
    ax_0.set(title=f'Loss over Iterations', xlabel='$t$', ylabel='$\\ell(x^{(t)}, p)$')
    ax_0.legend()
    ax_0.grid('on')
    ax_0.set_yscale('log')
