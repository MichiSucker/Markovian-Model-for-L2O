...
class TestStoppingCriterion(unittest.TestCase):
    ...
    def test_call_combined_criterion(self):

        def f_1(x: torch.Tensor, y: torch.Tensor):
        ...
        criterion = CombinedCriterion([stopping_criterion_1, stopping_criterion_2])

        x_1 = torch.Tensor([1, 1])
        x_2 = torch.Tensor([2, 2])
        self.assertTrue(criterion(x_1, x_2))
        self.assertFalse(criterion(x_1, x_1))
